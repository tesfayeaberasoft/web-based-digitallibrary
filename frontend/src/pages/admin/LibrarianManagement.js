...
const LibrarianManagement = () => {
  ...
  useEffect(() => {
    fetchUsers();
  }, [page, searchQuery, statusFilter, showAllUsers]);
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        page: page.toString(),
        limit: '10',
        role: showAllUsers ? '' : 'librarian', // Show all users or just librarians
  ...
  const handleOpenDialog = (dialogType, user = null) => {
    ...
    if (dialogType === 'add') {
      setFormData({
        ...
        phone: '',
        address: '',
        role: 'librarian', // Always set to 'librarian'
        status: 'active',
        department: '',
        employee_id: '',
        shift: 'morning'
      ...
      setFormData({
        ...
        role: 'librarian', // Always 'librarian'
        status: user.status,
    ...
    } else if (dialogType === 'demote' && user) {
      setDemotionReason('');
      setFormData({
        role: 'user'
      });
  ...
  const handleDemoteUser = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      console.log('Demoting librarian:', selectedUser.id, 'to user');
      ...
      if (demotionReason.trim()) {
        demotionData.demotion_reason = demotionReason.trim();
      }
      ...
      if (response.data.success) {
        setSuccess(`Librarian ${selectedUser.full_name} has been demoted to user successfully`);
        handleCloseDialog();
        fetchUsers();
      } else {
      ...
      <Box>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
            <Box>
              ...
              <Typography variant="h6" color="text.secondary">
                Manage librarians, promote users, and control staff permissions
              </Typography>
            </Box>
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
              <FormControlLabel
                ...
                label={showAllUsers ? "Showing All Users" : "Showing Librarians Only"}
              />
              <Button
                variant="contained"
                startIcon={<PersonAdd />}
          ...
          <Card sx={{ mb: 3, bgcolor: '#f5f5f5' }}>
            <CardContent>
              ...
              <Typography variant="body2">
                • Toggle to show all users or just librarians<br/>
                • Super-admins have full control: add, edit, delete, promote, demote<br/>
                • Promote regular users to librarians<br/>
                • Demote librarians back to regular users<br/>
                • Manage employment details: department, shift, employee ID<br/>
                • Selected User: {selectedUser ? `${selectedUser.full_name} (ID: ${selectedUser.id}, Role: ${selectedUser.role})` : 'None'}<br/>
                • Showing: {showAllUsers ? 'All Users' : 'Librarians Only'}
              </Typography>
            </CardContent>
          </Card>
        )}
        ...
        >
          ...
          {selectedUser?.role === 'librarian' ? (
            <MenuItem onClick={() => {
              handleOpenDialog('demote', selectedUser);
            }} sx={{ color: 'warning.main' }}>
              <PersonOff sx={{ mr: 2, fontSize: 20 }} />
              Demote to User
            </MenuItem>
          ) : (
            <MenuItem onClick={() => {
        ...
        <Dialog open={openDialog === 'demote'} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
          ...
          <DialogContent>
            <DialogContentText sx={{ mb: 2 }}>
              Demote librarian <strong>{selectedUser?.full_name}</strong> to regular user role. This will remove their staff privileges and librarian access.
            </DialogContentText>
            ...
            <Alert severity="warning" sx={{ mb: 2 }}>
              ...
              <Typography variant="body2" component="ul" sx={{ mt: 1, pl: 2 }}>
                <li>Remove librarian privileges</li>
                <li>Revoke access to staff functions</li>
                <li>Change role to regular library user</li>
              </Typography>
            </Alert>
            
            <TextField
              fullWidth
              label="Reason for Demotion (Optional)"
              multiline
              rows={3}
              value={demotionReason}
              onChange={(e) => setDemotionReason(e.target.value)}
              placeholder="Enter the reason for this demotion..."
              helperText="This reason will be recorded for administrative purposes"
            />
        ...
        {/* Promote User to Librarian Dialog */}
            ...
            <Grid container spacing={2} sx={{ mt: 1 }}>
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  helperText="This reason will be recorded for HR purposes"
                />
              </Grid>
            </Grid>
